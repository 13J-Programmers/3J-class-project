...
namespace Player.Action {
	...
	public class KeyAction : PlayerAction {
		...
		void Update() {
			...
			// Move Block
			if (Input.GetKey("up")) {
				Vector3 forward = mainCamera.TransformDirection(Vector3.forward) * speed;
				blockController.MoveBlock(forward);
			}
			else if (Input.GetKey("down")) {
				Vector3 back = mainCamera.TransformDirection(Vector3.back) * speed;
				blockController.MoveBlock(back);
			}
			else if (Input.GetKey("right")) {
				Vector3 right = mainCamera.TransformDirection(Vector3.right) * speed;
				blockController.MoveBlock(right);
			}
			else if (Input.GetKey("left")) {
				Vector3 left = mainCamera.TransformDirection(Vector3.left) * speed;
